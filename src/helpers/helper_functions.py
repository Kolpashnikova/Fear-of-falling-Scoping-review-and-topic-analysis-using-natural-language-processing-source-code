...
def get_list(texts, stop_words, processing_choice='nouns', N_s = 100,
    ...
    filename = path_to_file + processing_choice + '_' + str(cutoff_val) + '.csv'

    if os.path.exists(filename):
        lst = pd.read_csv(filename).drop('Unnamed: 0', axis=1).fillna('').values.tolist()
        ...
        pd.DataFrame(list_texts_filter).to_csv(filename)
...
def get_top_n_words(corpus, n=None, vect='CountVectorizer'):
    ...
    words_freq = Counter({word: bag_of_words.getcol(idx).sum() for word, idx in vec.vocabulary_.items()})
    if n is None:
        return [word for word, _ in words_freq.most_common()]
    else:
        return [word for word, _ in words_freq.most_common(n)]

def get_topics_from_model(model, vect, n_top_words):
    feature_names = vect.get_feature_names()
    num_topics = len(model.components_)
    topics = [ [ feature_names[i] for i in topic.argsort()[: -n_top_words - 1 : -1] ] for topic in model.components_ ]
    return topics
    ...
    for t in texts:
        ...
        if paradigm == 'nouns':
            ...
            result.append([token.lemma_.lower() for token in doc 
                           if (token.pos_ in ['NOUN']) & (token.lemma_.lower() not in stop_words) and not token.text.isupper() and not token.is_stop])
